...
---
As my previous blogposts already covered how to set up a private Maven repository, you might wonder "Why again a Maven blogpost?". Well that's a very fair question as Artifcatory was already free and ridiculously easy to set up.

However, Artifactory still requires you to solve two main problems yourself:

1. Provide a backup mechanism, to ensure you never ever loose releases.
2. Allow remote access to Artifactory so you can also build projects from outside your local network.

Arguable, both problems shouldn't pose a major challenge in this day and age. But as they still require effort/cost to set up, my DIY self challenged me to come up with an easy solution to both of these problems.

Therefore this Blogpost will demonstrate how easy it is to configure Bitbucket to have an essentially free private remote Maven repository. Even more, I will provide a fully integrated Gradle script to handle all the uploading complexity for you.

## Preface
While you could also use Github to host your Maven repository (or any other git repo for that matter), I choose Bitbucket for this example as it offers [free private repositories](https://bitbucket.org/product/pricing) for small teams. Furthermore it offers an API which we will need for some of wizardry later on.

All of the source code of this blogpost is also available on Github. It does require some configuration to connect it to Bitbucket, but it will straightforward kickstart you.
- explain why Bitbucket: free private repos, remote access, backups
- discuss about Github Example

## Bitbucket
### About
// do I need an about here

### Basic configuration
First of all login to Bitbucket and create a new private repository called `maven_repository`.
