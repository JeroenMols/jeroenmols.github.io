---
layout: post
title: Reducing your apps method count
published: true
comments: true
img: img/blog/methodcount/methodcount.png
---
As green field projects are a rare breed, chances are that you inherited a legacy code base. If you're as lucky as me, that code base has over 65k methods causing the build times to be boringly slow.
...
Note that you can interact with the graphic by clicking the one above.
...
Normally I recommend never to optimize unless you have a problem. But with method counts, I really advice you to consider the method count of a library before you start using it for two reasons:

1. Replacing libraries in existing apps can be very challenging, if not even almost impossible.
2. Many developers are using huge libraries just for way to simple things. (`Strings.isNullOrEmpty()` anyone?) Note that you don't need to use a library for everything!
...
If it's not feasible to replace an existing library or to run Proguard all the time, then still all is not lost. Because if Proguard can strip out code during a release build, why not use it ahead of time to create a modified version of a library with fewer methods?
...
Next we'll create a proguard configuration that keeps all packages that are currently being used. Make sure to also add the `-dontoptimize` and `-dontobfuscate` flags!
...
Now we'll use a little Gradle script that takes a library as an input, runs Proguard on it an creates a new library as an output. If you're interested I advice you look at the source code here, which is based on a script by [@mr_ligi](https://github.com/ligi/shrinkGuava). This gradle script outputs a shrinked library version, which can be copied to the libs folder of your project.

Looking at our example, we gained fine grained methods
...
Now we can start creating a modified version of Guava with fewer packages/classes and hence also methods. For this I used a neatly written Gradle script that takes



Obviously your mileage will vary and will work best for low coupled, highly cohesive libaries.

Well this is exactly what I did for one of the projects that I worked on recently, that made use of some parts of the Guava library.

This is exactly what I did for one of the projects I recently worked on


// TODO

Total methods in app-debug.apk: 16259 (24.81% used)
Total fields in app-debug.apk:  7101 (10.84% used)
Methods remaining in app-debug.apk: 49276
Fields remaining in app-debug.apk:  58434

Total methods in app-debug.apk: 55162 (84.17% used)
Total fields in app-debug.apk:  30216 (46.11% used)
Methods remaining in app-debug.apk: 10373
Fields remaining in app-debug.apk:  35319

Removing/replacing libraries usually isn't a short term solution as they are used everywhere throughout the app be a daunting task for large projects. While Proguard can certainly increase your mileage, you would also have to run it on every debug builds.
